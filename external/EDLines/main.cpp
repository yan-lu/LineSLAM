...
int main(){
  ...
  cv::Mat im = cv::imread("house.pgm",0);
  cv::namedWindow("img");
  cv::imshow("img",im);
  cv::waitKey(100);


  uchar* p1 = new uchar[im.rows*im.cols];	
  uchar* srcImg1 = p1;
  timer.Start();
  ...
  timer.Stop();
  cout<<"copying image using at \t"<<timer.ElapsedTime()<<endl;

	
   uchar* imdata = (uchar*) im.data;
   size_t step = im.step;
   size_t elesize = im.elemSize();

   uchar* p2 = new uchar[im.rows*im.cols];	
   uchar* srcImg2 = p2;	
   timer.Start();
   for(int i=0; i<im.rows; ++i) {
      uchar* pr = im.ptr<uchar>(i);
      for(int j=0; j<im.cols; ++j) {
	   //*(p2++) = imdata[step*i+j*elesize];
	   *(p2++) = pr[j];		
	}
  }
  timer.Stop();
  cout<<"copying image using ptr \t"<<timer.ElapsedTime()<<endl;

   uchar* p3 = new uchar[im.rows*im.cols];	
   uchar* srcImg3 = p3;	
   timer.Start();
   for(int i=0; i<im.rows; ++i) {
      for(int j=0; j<im.cols; ++j) {
	   *(p3++) = imdata[step*i+j*elesize];
	   }
  }
  timer.Stop();
  cout<<"copying image using step \t"<<timer.ElapsedTime()<<endl;
  cout<<"image step is "<<step<<endl;
 ...
 if (ReadImagePGM(str, (char **)&srcImg, &width, &height) == 0){
  ...
  width = im.cols; height = im.rows;	
  timer.Start();
  lines = DetectLinesByED(srcImg3, width, height, &noLines);
  timer.Stop();
  printf("<%d> line segments detected in <%4.2lf> ms\n", noLines, timer.ElapsedTime());
