...
/*  double* tmp0 = (double *) malloc(n_tuple->dim * n_tuple->max_size* sizeof(double) );

  memcpy(tmp0, n_tuple->values, n_tuple->dim *n_tuple->size* sizeof(double));
//  double* tmp = (double *) realloc( (void *) n_tuple->values,
//                     n_tuple->dim * n_tuple->max_size * sizeof(double) );
   cout<<"1.2"<<endl;
   cout<<"1.3"<<endl;
  n_tuple->values = tmp0;
...
ntuple_list lsd_scale(image_double image, double scale)
{
  ...
  double quant = 2.0;       /* Bound to the quantization error on the
                                gradient norm.                                */
  double ang_th = sysPara.lsd_angle_th;     /* Gradient angle tolerance in degrees.  22.5          */
  double eps = 0.0;         /* Detection threshold, -log10(NFA).              */
  double density_th = sysPara.lsd_density_th; /* Minimal density of region points in rectangle. 0.7*/
