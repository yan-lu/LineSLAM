...
*/
  ...
  QApplication app(argc, argv, ParameterServer::instance()->get<bool>("use_gui"));  // ylu: randome seed changed
  ...
  Graphical_UI* gui = NULL;
	if (app.type() == QApplication::GuiClient){
      gui = new Graphical_UI();     // ylu: random seed changed
      gui->show();
...
#ifdef USE_LINES
  srand(1);
  ///////////// changing the ROS log level /////////////////
  if(ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Error) ) {
     ros::console::notifyLoggerLevelsChanged();
   }
   sysPara.init(); // ROS parameter server is too costly to call frequently. global varaible much faster
#endif

#ifdef ALLOW_LOAD_RAWDATA  
  std::string rawdata_name = ParameterServer::instance()->get<std::string>("raw_data_directory_name");
  if(!rawdata_name.empty()) 
  {
    QObject::connect(&listener, SIGNAL(bagFinished()), &app, SLOT(quit()));
    QObject::connect(&listener, SIGNAL(bagFinished()), &qtRos, SLOT(quitNow()));
    listener.loadRawDataFromGUI(rawdata_name.c_str());
  }

  std::string rawdata_listfile = ParameterServer::instance()->get<std::string>("raw_data_dir_list_file");
  if(!rawdata_listfile.empty()) 
  {
    QObject::connect(&listener, SIGNAL(bagFinished()), &app, SLOT(quit()));
    QObject::connect(&listener, SIGNAL(bagFinished()), &qtRos, SLOT(quitNow()));
    listener.loadRawDataListFromGUI(rawdata_listfile);
  }
#endif
