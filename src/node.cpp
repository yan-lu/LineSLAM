...
#ifdef USE_LINES
void write_line3d_to_file(vector<FrameLine> lines, string fname) 
{  
  ...
  for(int i=0; i<lines.size(); ++i) {
    if(lines[i].haveDepth) {
      ...
      for(int j=0; j<3; ++j) {
        for(int k=0; k<3; ++k) {
          file << lines[i].line3d.rndA.cov.at<double>(j,k) <<'\t';
        }
      }
      for(int j=0; j<3; ++j) {
        for(int k=0; k<3; ++k) {
          file << lines[i].line3d.rndB.cov.at<double>(j,k) <<'\t';
        }
      }
    }
    file<<'\n';
...
#ifdef MODEL_ASYNCH
  asynch_time_diff_sec_(time_diff),
#else
  asynch_time_diff_sec_(0),
#endif  
  initial_node_matches_(0)
{
  ...
  cv::Mat camInMat = (cv::Mat_<double>(3,3)<<fx, 0, cx, 0, fy, cy, 0, 0, 1);
  K = (cv::Mat_<double>(3,3)<<fx, 0, cx, 0, fy, cy, 0, 0, 1); // initialize global variable

  double line_2d_len_thres =  ps->get<double>("line_2d_len_thres");
  double collin_pts_ratio = ps->get<double>("collin_pts_ratio");
  double line_3d_len_thres_m = ps->get<double>("line_3d_len_thres_m");
  double depth_scaling_factor = ps->get<double>("depth_scaling_factor");
  std::string line_detect_algorithm = ps->get<std::string>("line_detect_algorithm");

  QFuture<void> future_linedetect 
  = QtConcurrent::run(boost::bind(&Node::detect3DLines, this, gray_img, depth, line_2d_len_thres, camInMat,collin_pts_ratio, line_3d_len_thres_m, depth_scaling_factor,line_detect_algorithm));
...
#ifdef USE_LINES
  if(!future_linedetect.isFinished()){   
    future_linedetect.waitForFinished(); //Wait if GraphManager ist still computing. 
  }
#endif  

//  write_line3d_to_file(lines, "/home/lu/3dline.txt"); exit(0);
}
...
#ifndef USE_LINES // original function definition in RGBDSLAM 
        ...
        if(found_transformation){
          ...
          if(((int)this->id_ - (int)older_node->id_) <= 1){ //Apply icp only for adjacent frames, as the initial guess needs to be in the global minimum
            ...
            if(found_transformation){
              ...
              mr.edge.informationMatrix =   Eigen::Matrix<double,6,6>::Identity(); // added by ylu, bug fixed: information was not set 
              mr.edge.id1 = older_node->id_;//and we have a valid transformation
              mr.edge.id2 = this->id_; //since there are enough matching features,
              mr.edge.transform = mr.final_trafo.cast<double>();//we insert an edge between the frames
...
#ifdef USE_LINES  // new function definition in LineSLAM
MatchingResult Node::matchNodePair(const Node* older_node)
{
  MatchingResult mr;
  try {
    ...
    const unsigned int min_matches = (unsigned int) ParameterServer::instance()->get<int>("min_matches");// minimal number of feature correspondences to be a valid candidate for a link
    this->featureMatching(older_node, &mr.all_matches);  
    bool adjacentMatchFlag = true;
    if (abs(this->id_-older_node->id_) > sysPara.adjacent_linematch_window)
      adjacentMatchFlag = false;
    this->lineMatching(older_node, adjacentMatchFlag, &mr.all_line_matches); // TODO: make it parallel to point matching 
    ROS_DEBUG_NAMED(__FILE__, "found %i inital point matches and %i inital line matches",(int) mr.all_matches.size(), (int)mr.all_line_matches.size());
    //-----------------------------------------
        
    if(mr.all_matches.size() + mr.all_line_matches.size() * sysPara.line_match_number_weight < min_matches) {
      if(this->id_-older_node->id_<=1)
        printf("Too few inliers between %i and %i for RANSAC method. Only %i point and %i line matches to begin with. Two nodes can't be connected...\n",
                 older_node->id_,this->id_,(int)mr.all_matches.size(), (int)mr.all_line_matches.size());
    } else {//All good for feature based transformation estimation

      double ransac_quality = 0; 
   // ========== line based motion estimation ===========  
  //    MyTimer tm; tm.start();
      found_transformation = getTransform_PtsLines_ransac(older_node, this, mr.all_matches, mr.all_line_matches, 
                                                       mr.inlier_matches, mr.inlier_line_matches, mr.ransac_trafo, mr.rmse);      
  //    tm.end(); cout<<"get transform \t\t"<<tm.time_ms<<" ms\n";
      //Statistics
      bool apply_const_vel = false;
      float nn_ratio = 0.0;
      if(found_transformation){
        mr.final_trafo = mr.ransac_trafo;
        for(unsigned int i = 0; i < mr.inlier_matches.size(); i++){
        ...
        mr.edge.informationMatrix = Eigen::Matrix<double,6,6>::Identity()
                            *((mr.inlier_matches.size()+mr.inlier_line_matches.size()*sysPara.line_match_number_weight)/(mr.rmse*mr.rmse)); //TODO: What do we do about the information matrix? Scale with inlier_count. Should rmse be integrated?)
        ...
        ROS_INFO("RANSAC found a %s transformation with %i point and %i line inlier matches with average ratio %f", \
                   found_transformation? "valid" : "invalid", (int) mr.inlier_matches.size(), mr.inlier_line_matches.size(), nn_ratio);
        ...
        nn_ratio /= mr.all_matches.size();
        ROS_INFO("RANSAC found no valid trafo, but had initially %d feature matches with average ratio %f",(int) mr.all_matches.size(), nn_ratio);
        if(((int)this->id_ - (int)older_node->id_) <= 1){        
          apply_const_vel = true;
        }
#ifdef USE_PCL_ICP          
        if(((int)this->id_ - (int)older_node->id_) <= 1){ //Apply icp only for adjacent frames, as the initial guess needs to be in the global minimum
          printf("PCL_ICP called for between %i and %i.  Becasue %i correspondences, but no transfo found.\n",
                 older_node->id_,this->id_,(int)mr.all_matches.size());
          mr.icp_trafo = Eigen::Matrix4f::Identity();
          int max_count = ParameterServer::instance()->get<int>("gicp_max_cloud_size");
          pointcloud_type::Ptr tmp1(new pointcloud_type());
          ...
          if(found_transformation){
            ...
            mr.edge.id2 = this->id_; //since there are enough matching features,
            mr.edge.transform = mr.final_trafo.cast<double>();//we insert an edge between the frames
            cout<<"from icp "<<1000*(-mr.final_trafo.block(0,0,3,3).transpose() * mr.final_trafo.block(0,3,3,1)).transpose()<<endl;
          }
        }
#endif
      // ***** use const velocity (vel computed after addNode in graph_manager) 
        if(apply_const_vel) {
          cout<<":::::: const velocity ::::::\n";
          double dt = this->header_.stamp.toSec() - older_node->header_.stamp.toSec();
          Eigen::Vector3f tr = older_node->pose.block(0,0,3,3).transpose()*(dt*older_node->vel);
          cout<<"relative t = "<<tr.transpose()<<endl;
          mr.ransac_trafo << 1, 0, 0, tr(0),
                           0, 1, 0, tr(1),
                           0, 0, 1, tr(2),
                           0, 0, 0, 1;
          found_transformation = true;
          ...
          mr.edge.id1 = older_node->id_;//and we have a valid transformation
          mr.edge.id2 = this->id_; //since there are enough matching features,
          mr.edge.transform = mr.final_trafo.cast<double>();//we insert an edge between the
        }
      }
    } 
    if(found_transformation) {
...
#ifdef USE_LINES
unsigned int Node::lineMatching(const Node* other, const bool adjacentFrame, std::vector<cv::DMatch>* matches) const
// if two nodes are adjacent, use descriptors and distance constraint, otherwise, only use descriptors
{  
  double lineDistThresh,  lineAngleThresh, descDiffThresh,  lineOverlapThresh, ratio_dist_1st2nd;
  if(adjacentFrame) {
    lineDistThresh  = 45; // in pixel, a large threshold to rule out inpossible matches
    lineAngleThresh = 30 * PI/180; // 30 degree
    descDiffThresh   = 0.85;
    lineOverlapThresh = 0; // pixels
    ratio_dist_1st2nd = 0.7;    
  } else {
    lineDistThresh  = 80; // in pixel, a large threshold to rule out inpossible matches
    lineAngleThresh = 30 * PI/180; // 30 degree
    descDiffThresh   = 0.7;
    lineOverlapThresh = -1; // pixels
    ratio_dist_1st2nd = 0.7;  
  }
 
  const std::vector<FrameLine>& f1 = this->lines;
  const std::vector<FrameLine>& f2 = other->lines;
  if(f1.size()==0 || f2.size()==0) {
    return 0;
  }
  // mutual descriptor distance stored in a matrix
  cv::Mat descDiff = cv::Mat::zeros(f1.size(), f2.size(), CV_64F)+100;
  ...
  for(int i=0; i<f1.size(); ++i) {    
    for(int j=0; j<f2.size(); ++j) {
      ...
      {
        descDiff.at<double>(i,j) = cv::norm(f1[i].des - f2[j].des);
      }
    }
  }
  
  for(int i=0; i<descDiff.rows; ++i) {
    vector<int> onePairIdx;
    double minVal;
    cv::Point minPos;
    cv::minMaxLoc(descDiff.row(i),&minVal,NULL,&minPos,NULL);
    if (minVal < descDiffThresh) {
      double minV;
      cv::Point minP;
      cv::minMaxLoc(descDiff.col(minPos.x),&minV,NULL,&minP,NULL);     
      if (i==minP.y) {    // mutual minimum 
        //further check distance ratio
        double rowmin2 = 100, colmin2 = 100;
        for(int j=0; j<descDiff.cols; ++j) {
          if (j == minPos.x) continue;
          if (rowmin2 > descDiff.at<double>(i,j)) {
            rowmin2 = descDiff.at<double>(i,j);
          }
        }
        for(int j=0; j<descDiff.rows; ++j) {
          if (j == minP.y) continue;
          if (colmin2 > descDiff.at<double>(j,minPos.x)) {
            colmin2 = descDiff.at<double>(j,minPos.x);
          }
        }
        if(rowmin2*ratio_dist_1st2nd > minVal && colmin2*ratio_dist_1st2nd > minVal) { // match found
          cv::DMatch match;
          match.queryIdx = i;
          match.trainIdx = minPos.x;
          match.distance = minVal; 
          if(this->lines[i].haveDepth && other->lines[minPos.x].haveDepth) {
            matches->push_back(match);
          }
        }
      }
 ...
 //printf("Between frame %d and %d, %d line matches are found\n", this->id_, other->id_, matches->size());
  return matches->size();
}
