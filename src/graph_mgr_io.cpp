...
#ifdef USE_LINES
      logTransform(et_out, pose, node->header_.stamp.toSec());
#else // in the original rgbdslam, an offset is added, which makes it incompatible with the groundtruth frame.
      tf::StampedTransform base2points = node->getBase2PointsTransform();//get pose of base w.r.t current pc at capture time
                           ...
                           const std::vector<cv::KeyPoint>& older_keypoints)
{
    ...
    for(unsigned int mtch = 0; mtch < matches.size(); mtch++) {
        ...
        if(newer_idx > newer_keypoints.size() || newer_keypoints.size()<1) break;//Added in case the feature info has been cleared
        q = newer_keypoints[newer_idx].pt;
        if(earlier_idx > older_keypoints.size() || older_keypoints.size() < 1) break;//Added in case the feature info has been cleared
