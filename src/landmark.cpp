...
//   old_node->visible_landmarks.insert(lm.id);
  ...
  if  (lm_1->observations.size() >= lm_2->observations.size()){
   ...
   //ylu: Bug found. erasing lm_id_new from the landmarks vector will cause 
   // all elements after it to be shifted towards the vector head. 
   // This would result in error later when querying a landmark with their ids, 
   // because their ids don't correspond to their real position/index in the vector.
