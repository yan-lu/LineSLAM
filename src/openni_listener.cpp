...
#ifdef USE_LINES
  tm.end(); cout<<"Processing bag file uses "<<tm.time_s<<" sec"<<endl;
#endif
...
#ifdef USE_LINES
  noCloudCameraCallback(visual_img, depth_float_img, depth_mono8_img_, visual_img_msg->header, 
  #ifdef MODEL_ASYNCH
                        abs(depth_img_msg->header.stamp.toSec() - visual_img_msg->header.stamp.toSec()),
  #endif
                        cam_info_msg);// use rgb-timestamp
#else  
                               ...
                               const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  ...
  //######### Main Work: create new node ##############################################################
  Q_EMIT setGUIStatus("Computing Keypoints and Features");
  Node* node_ptr = new Node(visual_img, depth, depth_mono8_img, cam_info, depth_header, 
#ifdef MODEL_ASYNCH
    time_diff_sec,
#endif
...
{
  /////////// Use QFuture to ensure sequential processing of frames ////////////
  if(!future_.isFinished()){
  ...
  cv::Mat canv_line = visual_img.clone();
  for(int i=0; i< node_ptr->lines.size(); ++i) {
    if(node_ptr->lines[i].haveDepth) 
      cv::line(canv_line, node_ptr->lines[i].p, node_ptr->lines[i].q, cv::Scalar(0,100,0), 2);
    else
      cv::line(canv_line, node_ptr->lines[i].p, node_ptr->lines[i].q, cv::Scalar(100,0,0), 2);
  }  
#endif
...
#ifdef USE_LINES      
      visualize_images(canv_line, depth_mono8_img_);
#else
...
#ifdef ALLOW_LOAD_RAWDATA
void OpenNIListener::loadRawDataFromGUI(QString dir)
{
  QtConcurrent::run(this, &OpenNIListener::loadRawData, dir.toStdString(), true);
}

void OpenNIListener::loadRawDataListFromGUI(std::string filename) 
{
  QtConcurrent::run(this, &OpenNIListener::loadRawDataList, filename);
}

void OpenNIListener::loadRawDataList(std::string filename) {
  ...
  if (file.is_open())  {
    for(int i=0; i<max_n; ++i) {
      std::string datapath;
      getline(file, datapath);
      if(datapath.size()<1)
        break; 
      else
        dirlist.push_back(datapath);
    }
  } else {
    std::cerr<<"error: dir list file not open."; exit(0);
  }
  file.close();

  for(int idx_dir = 0; idx_dir < dirlist.size(); ++idx_dir) {
    std::cout<<"########### loading in "<< dirlist[idx_dir] << "############\n";
    graph_mgr_->resetGraphFromOutside();
    loadRawData(dirlist[idx_dir], false);
...
void OpenNIListener::loadRawData(std::string dirname, bool alone)
{
#ifdef USE_LINES  
  MyTimer tm; tm.start();
#endif
  ...
  std::string syncname = dirname+"/syncidx.txt";
  std::ifstream fsync(syncname.c_str());
  int i=0;
  if (fsync.is_open())  {
    std::string tmp;
  ...
  Q_EMIT progress(4, "Processing raw data", 1);
  do{ 
    usleep(150);
    if(!ros::ok()) return;
  } while(pause_);

  for(int file_idx = 0; file_idx < list.size(); ++file_idx) {
    std::string rgbname = dirname + "/" + list[file_idx][1], 
           depthname = dirname + "/" + list[file_idx][3];
    cv::Mat color_img = cv::imread(rgbname, 1); // default opencv coding is BGR
    cv::Mat depth_img = cv::imread(depthname,CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH);     
    cv::Mat depth_float_img;
    depth_img.convertTo(depth_float_img, CV_32FC1); // the resulting pixel value is depth in meters 
    for(int i=0; i<depth_img.rows; ++i) {
      for(int j=0; j<depth_img.cols; ++j) {
        if(depth_float_img.at<float>(i,j) < 1e-5)
          depth_float_img.at<float>(i,j) = std::numeric_limits<double>::quiet_NaN();
      }
    }

    depth_float_img = depth_float_img /5000.0;

    double timestamp_color = atof(list[file_idx][0].c_str());
    double timestamp_depth = atof(list[file_idx][2].c_str());

    graph_mgr_->data_name = dirname.substr( dirname.size()-5, 5);

    // ====== simulate header and cam_info ======
    std_msgs::Header rgb_header;
    rgb_header.stamp.fromSec(timestamp_color);    
    rgb_header.frame_id = 1; //num2str(file_idx);
    const sensor_msgs::CameraInfoPtr rgbcam(new sensor_msgs::CameraInfo());
    rgbcam->K[0] = 525;
    rgbcam->K[4] = 525;
    rgbcam->K[2] = 319.5;
    rgbcam->K[5] = 239.5;
    rgbcam->K[8] = 1; 
    const sensor_msgs::CameraInfoConstPtr rgbcam_info(rgbcam);
    

    if(file_idx < ps->get<int>("skip_first_n_frames") || file_idx % ps->get<int>("data_skip_step") != 0) { 
    // If only a subset of frames are used, skip computations but visualize if gui is running
      ROS_INFO_THROTTLE(1, "Skipping Frame %i because of data_skip_step setting (this msg is only shown once a sec)", file_idx);
      if(ps->get<bool>("use_gui")){//Show the image, even if not using it
        cv::Mat visual_img =  color_img;
      }
      continue;  
    } 
    
    if(color_img.rows != depth_float_img.rows || color_img.cols != depth_float_img.cols){
      ROS_ERROR("depth and visual image differ in size! Ignoring Data");
      return;
    }
    ...
    if(pause_ && !getOneFrame_){ 
      if(ps->get<bool>("use_gui")){     
        Q_EMIT newVisualImage(cvMat2QImage(color_img, 0)); //visual_idx=0
        Q_EMIT newDepthImage (cvMat2QImage(depth_mono8_img_,1));//overwrites last cvMat2QImage
      }
      continue;
    }
    noCloudCameraCallback(color_img, depth_float_img, depth_mono8_img_, rgb_header,
#ifdef  MODEL_ASYNCH
      abs(timestamp_color - timestamp_depth),
#endif    
      rgbcam_info);
  }
...
#ifdef USE_LINES  
  tm.end(); 
  std::cout<<"This processing time is "<<tm.time_s<<" sec.\n";
#endif
