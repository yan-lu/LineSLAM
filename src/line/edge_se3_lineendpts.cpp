...
using namespace std;
Eigen::Vector3d normalized_pt2line_vec_0 (const Eigen::Vector6d& line, const Eigen::Vector3d& pt, const Eigen::Matrix3d& pt_cov) {
...
//	cout<<"computed\n " << D_invsqrt * svd.matrixU().transpose() <<endl;

	return Ap+ t*(Bp-Ap);
}

Eigen::Vector3d normalized_pt2line_vec (const Eigen::Vector6d& line, const Eigen::Vector3d& pt, const Eigen::Matrix3d& pt_am) {
// normalized by pt uncertainty
// return the vector from pt to the closest point on the line, vector direction is meaningless, but norm is of interest.
		
	Eigen::Vector3d A(line(0),line(1),line(2));
	Eigen::Vector3d B(line(3),line(4),line(5));
	Eigen::Vector3d Ap = pt_am * (A - pt);
	Eigen::Vector3d Bp = pt_am * (B - pt);
	double t = - Ap.dot(Bp-Ap)/((Bp-Ap).dot(Bp-Ap));
	return Ap+ t*(Bp-Ap);
}

Eigen::Vector3d closetPtonLine (const Eigen::Vector6d& line, const Eigen::Vector3d& pt) {
...
namespace g2o {
	...
	//	Vector3d normalized_pt2line_vec1_0 = normalized_pt2line_vec_0(ln, measpt1, endptCov.block<3,3>(0,0));
	//	Vector3d normalized_pt2line_vec2_0 = normalized_pt2line_vec_0(ln, measpt2, endptCov.block<3,3>(3,3));

	//	Vector3d normalized_pt2line_vec1 = normalized_pt2line_vec(ln, measpt1, endpt_AffnMat.block<3,3>(0,0));
	
	//	Vector3d normalized_pt2line_vec2 = normalized_pt2line_vec(ln, measpt2, endpt_AffnMat.block<3,3>(3,3));
