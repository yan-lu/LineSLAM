...
//////////////////   if use lines for vslam //////////////
#ifdef USE_LINES   
public:  
  double asynch_time_diff_sec_; // time difference between rgb and depth frame
  Eigen::Matrix4f pose; // as in g2o estimate, updated whenever graph optimized.
  Frame frame;
  Eigen::Vector3f vel; // m/s in WCS at this frame
  std::vector<FrameLine> lines;
  void detectFrameLines(const cv::Mat& gray, double line2d_len_thres, int method = 0);
  void extractLineDepth(const cv::Mat& depth_float, const cv::Mat& K, double ratio_collin_pts, double line3d_len_thres, double d_scaling);
  
  //// combo of detect and extract
  //// detect image line segments, extract depth, compute MSLD, MLE and cov only for 3d lines for fast speed.
  void detect3DLines(const cv::Mat& gray_uchar, const cv::Mat& depth_float, double line2d_len_thres, 
                      const cv::Mat& K,double ratio_of_collinear_pts, double line_3d_len_thres_m, double depth_scaling, string algorithm);
  unsigned int lineMatching(const Node* other,  const bool adjacentFrame, std::vector<cv::DMatch>* matches) const; // apply to non-adjacent frames
