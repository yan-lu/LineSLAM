...
// return a list of nodes for new_node to be compared with
...
{    
       ...
       ps->get<double>("min_rotation_degree") > 0.0)
    {
     ...
     //       cout<<"after add to g2o, position is "<< v->estimate().matrix()<<endl;
          ...
          } else { // couldn't add to g2o graph.
      ...
      else { // didn't find transformation
    ...
    if (ps->get<bool>("concurrent_edge_construction")) 
    {
        ...
        for (int i = 0; i < results.size(); i++) 
        {
            ...
            if (mr.edge.id1 >= 0 ) {
              ...
              if (isSmallTrafo(mr.edge.transform, delta_time.toSec()) && // true means speed is not over a limit
    ...
    else if(!found_trafo && keep_anyway) //no odometry data and visual odometry esitmation, but have to add to graph
                                         // use: Constant position assumption
    { 
      LoadedEdge3D odom_edge;
      ...
      ROS_ERROR("No valid transformation: Using constant position assumption.");
      odom_edge.informationMatrix = Eigen::Matrix<double,6,6>::Identity() * 1;//e-9; // bound the position within xx distance 
      odom_edge.informationMatrix(3,3) = 1e-100;  // facing direction has almost no bound
...
bool GraphManager::addNode(Node* new_node) 
{
  ...
  { //Success
    ...
    else //Mapping
    {
      ...
      graph_[new_node->id_] = new_node; //Node->id_ == Graph_ Index, ylu: this is acutally redundant with the one in function nodeComprisons
#ifdef USE_LINES
      //// ********* computer instant velocity for this frame **********
      int vid_new = new_node->vertex_id_;
      int id_old = max(0, new_node->id_ - 5);
      int vid_old = -1;
      if(graph_.find(id_old)!=graph_.end()){
        vid_old = graph_[id_old]->vertex_id_;
      } else {
        cout<<"not find old node in graph...";  exit(0);
      }     
      g2o::VertexSE3* vn = dynamic_cast<g2o::VertexSE3*> (optimizer_->vertex(vid_new) );
      g2o::VertexSE3* vo = dynamic_cast<g2o::VertexSE3*> (optimizer_->vertex(vid_old) );
      if(vn&&vo) {
        double dt = abs(new_node->header_.stamp.toSec() - graph_[id_old]->header_.stamp.toSec());
        Eigen::Vector3d linvel = (vn->estimate().matrix() - vo->estimate().matrix()).block(0,3,3,1)/dt;
        new_node->vel = linvel.cast<float>();
        //cout<<"node "<<new_node->id_<<", vel="<<new_node->vel.transpose()<<", "<<new_node->vel.norm()<<endl;
      } else {
        cout<<"not found g2o vertex, error in computing vel ..\n"; exit(0);
      }
#endif
...
#ifdef USE_LINES      
      update_node_poses();
      // write cam poses to file
//      write_poses_2file("/home/lu/Desktop/poses_"+ data_name + ".txt");      
#endif
    } 
  }
  else //Unsuccesful 
...
#ifdef USE_LINES
// ******* delete point cloud in previous nodes except the last one, save memory ********
   if(ParameterServer::instance()->get<bool>("clear_past_point_cloud")){
      for (std::map<int, Node*>::iterator it=graph_.begin(); it!=graph_.end(); ++it){
        Node* mynode = it->second;
        if(mynode->id_ < graph_.size()-1){ // only keep pc for the latest node
          mynode->getMemoryFootprint(true);//print 
          mynode->clearPointCloud();
      //    if(!keyframe_ids_.contains(mynode->id_))          
            vector<FrameLine>().swap(mynode->lines);
        }
      }
  }
#endif 
...
#ifdef USE_LINES
void GraphManager::write_poses_2file(string filename) 
{
  // the output format is compatible with the groundtruth format from TUM datase
  ...
  for (std::map<int, Node*>::iterator it=graph_.begin(); it!=graph_.end(); ++it){
    ...
    Eigen::Matrix4d tf = v->estimate().matrix();
    Eigen::Vector4d q = r2q(tf.block(0,0,3,3));
    double ts = node->header_.stamp.toSec();
    of<<ts<<'\t'<<tf(0,3) <<'\t'<<tf(1,3) <<'\t'<<tf(2,3) <<'\t'
    <<q(1)<<'\t'<<q(2)<<'\t'<<q(3)<<'\t'<<q(0)<<'\n';
  }
}

void GraphManager::update_node_poses() 
{
  for (std::map<int, Node*>::iterator it=graph_.begin(); it!=graph_.end(); ++it){
    Node* node = it->second;
    if(!node->valid_tf_estimate_) {
    ...
    g2o::VertexSE3* v = dynamic_cast<g2o::VertexSE3*>(optimizer_->vertex(node->vertex_id_)); // TODO mutex ?
    node->pose = v->estimate().matrix().cast<float>(); // update pose
  }
}
...
void GraphManager::resetGraphFromOutside()
{
  resetGraph();
}
#endif